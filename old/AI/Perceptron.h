...
namespace ai{
    ...
    class Perceptron
    {
        public:
            ...
            const vector<double>& grad()    {   return _grad_param; }
...
//            }
    ...
    void _allocate_temporary(const Index& train_size)
    {
        ...
        _a_param = vector<double>(_a_size, 1);
        _z_param = vector<double>(_z_size, 1);
        _grad_param = vector<double>(_structure.total, 1);
        ...
        Index i = 0,
              n = _structure.hidden.size() - 1,
              param_size = 0,
              a_size = 0,
              z_size = 0;

        _a[0]    = Eigen::Map<Matrix>(&_a_param[a_size], train_size, _structure.input + 1);
        _z[0]    = Eigen::Map<Matrix>(&_z_param[z_size], train_size, _structure.hidden[0]);
        _grad[0] = Eigen::Map<Matrix>(&_grad_param[param_size], // Pointer to the first parameter
                                      _structure.input + 1,     // Number of row
                                      _structure.hidden[0]);    // Number of Column

        a_size += train_size * (_structure.input + 1);
        z_size += train_size * _structure.hidden[0];
        param_size += (_structure.input + 1) * _structure.hidden[0];

        for(i = 1; i < n; ++i)
        {
            _a[i]    = Eigen::Map<Matrix>(&_a_param[a_size], train_size, _structure.hidden[i] + 1);
            _z[i]    = Eigen::Map<Matrix>(&_z_param[z_size], train_size, _structure.hidden[i + 1]);
            _grad[i] = Eigen::Map<Matrix>(&_grad_param[param_size], _structure.hidden[i] + 1, _structure.hidden[i + 1]);

            a_size += train_size * (_structure.hidden[i] + 1);
            z_size += train_size * _structure.hidden[i + 1];
            param_size += (_structure.hidden[i] + 1) * _structure.hidden[i + 1];
        }

        _a[i]    = Eigen::Map<Matrix>(&_a_param[a_size], train_size, _structure.hidden[n] + 1);
        _z[i]    = Eigen::Map<Matrix>(&_z_param[z_size], train_size, _structure.output);
        _grad[i] = Eigen::Map<Matrix>(&_grad_param[param_size], _structure.hidden[n] + 1, _structure.output);
    }
